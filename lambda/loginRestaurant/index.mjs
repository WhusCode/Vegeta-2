...
async function dbOps(username) {
    ...
    // Execute the query
    const [rows] = await conn.execute('SELECT user_password, role_id FROM restaurant.user_info where user_name=?',[username]);
...
export const handler = async (event) => {
    ...
    if (password === JSON.stringify(db_password[0].user_password)) {
        ...
        let role = db_password[0].role_id;
        return {
            "isBase64Encoded": false,
            "statusCode": 200,
            "body": JSON.stringify({ message: "Login successful!", role: role }),
            "headers": {
                "Access-Control-Allow-Origin": "*",
                "Access-Control-Allow-Headers" : "Content-Type",
                "Access-Control-Allow-Methods": "OPTIONS,POST,GET"
